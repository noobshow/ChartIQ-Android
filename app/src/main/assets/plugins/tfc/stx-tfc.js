...
STX.TFC.prototype.updateValues=function(){
	...
	if(this.activeTrade=="market"){
		...
		price=this.quantityFromValue(this.elements.dragLineCenterPrice.innerHTML);
		if(this.activeInput=="shares"){
		...
		for(j=0;j<symbolList.length;j++){
			...
			for(i=0;i<openOrders.length;i++){
				...
				if(lastWasOco){
					...
					STX.newChild(tr, "TD", null, openOrder.displaySymbol?openOrder.displaySymbol:symbol);
				}
				price=0;
				if(openOrder.limit){
					...
					STX.newChild(tr, "TD", null, openOrder.debit>0?"@DB":"@CR");
					STX.newChild(tr, "TD", null, Math.abs(openOrder.debit));
					//price=openOrder.debit;
				}else if(openOrder.debit===0){
					STX.newChild(tr, "TD", null, "@EVEN");
...
/*		$$$(".stx-trade-positions").style.display="none";
	...
	if(this.positionIScroll===null){
		this.positionIScroll=STX.iscroll.newScroller(positionWrapper, {tap:true, scrollbars: true, interactiveScrollbars: true});
	}else{
		this.positionIScroll.refresh();
	}
	if(this.openOrderIScroll===null){
		this.openOrderIScroll=STX.iscroll.newScroller(openOrderWrapper, {tap:true, scrollbars: true, interactiveScrollbars: true});
	}else{
...
STX.TFC.prototype.confirmClosePosition=function(position){
	...
	var order={
		...
		tif:"DAY",
		pl:STX.money(pl, null, STX.convertCurrencyCode(position.currency))
	};
	if(this.config.skipConfirms){
		this.placeOrder(order);
	}else{
