...
function addSeries(symbol, hexColor, isComparison) {
	...
	console.log(symbol, hexColor, isComparison);
...
function getAddedStudies() {
	...
	for(var n in s) {
		...
		var inputs = JSON.stringify(sd.inputs, function(key, value){
			if (typeof value === 'object' && value !== null) {
				if (seen1.indexOf(value) !== -1) {
		...
		var outputs = JSON.stringify(sd.outputs, function(key, value){
			if (typeof value === 'object' && value !== null) {
